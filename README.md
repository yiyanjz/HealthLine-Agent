...
The only issue with this model is the slightly inaccurate collection of user email addresses, due to precision limitations. A reliable solution is to integrate Twilio to send an SMS to the user, allowing them to manually input the required information. This data can then be retrieved through a webhook, significantly improving the accuracy of the collected information. Due to recent changes in VAPI's workflow, I'm no longer able to make further adjustments, as the necessary nodes are no longer available.
...
- Full workflow video availble in files
